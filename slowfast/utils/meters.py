...
class SurgeryMeter(object):
    ...
    def update_grounding(self, preds, visn_att_mask, ground_bboxes, gt_bboxes, names, texts):
        ...
        try:
            for i in range(len(preds)):
                ...
                if any('phrase' in task for task in self.cfg.TASKS.TASKS):
                    batch_preds = preds[i][:,visn_att_mask[i]==1]
                    batch_preds = batch_preds[:len(gt_bboxes[i])].tolist()
                    assert len(batch_boxes)==len(batch_preds[0]), f"{len(batch_boxes)} and {len(batch_preds)}"
                else:
                    batch_preds = preds[i][visn_att_mask[i]==1].tolist()
                    assert len(batch_boxes)==len(batch_preds), f"{len(batch_boxes)} and {len(batch_preds)}"
                
                self.gt_boxes.append(gt_bboxes[i].tolist())
                self.ground_boxes.append(batch_boxes)
                self.ground_preds.append(batch_preds)
                self.texts.append(texts[i])
                self.ground_names.append(names[i])
        except:
            traceback.print_exc()
            breakpoint()
    ...
    def finalize_metrics(self, log=True):
        ...
        if self.mode == 'train':
            ...
            for idx, task in enumerate(self.tasks):
                out_name[task] = self.save_json(task, self.all_preds[task], self.all_boxes,  self.all_detect_names, 
                                                self.all_names, self.ground_preds, self.ground_boxes, self.gt_boxes,
                                                self.texts, self.ground_names)
    ...
    def save_json(self, task, preds, boxes, d_names, names, ground_preds, ground_boxes, groung_gt_boxes, texts, ground_names):
        """
        ...
        if task in ['actions', 'tools']:
            ...
            for pred, g_boxes, g_gt_box, text, name in zip(ground_preds, ground_boxes, groung_gt_boxes, texts, ground_names):
                task_key_name = 'prob_'+task
                save_dict = {'text': text, task_key_name: pred, 'bboxes': g_boxes, 'gt': g_gt_box}
                if name not in save_json_dict.keys():
                    save_json_dict[name] = {'grounds': [save_dict]}
                else:
                    save_json_dict[name]['grounds'].append(save_dict)
        else:
