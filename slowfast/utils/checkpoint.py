...
def load_ground_checkpoint(cfg,model):
    # breakpoint()
    with pathmgr.open(cfg.TRAIN.LANG_CHECKPOINT, "rb") as f:
        checkpoint = torch.load(f, map_location="cpu")
    
    if cfg.TRAIN.PRETRAIN == 'cross' or cfg.TRAIN.PRETRAIN == 'full':
        pre_train_dict = {k.replace("bert_encoder.model.bert.encoder.",""):v for k,v in checkpoint["model_state"].items()}
        cross_encoder_dict = model.cross_encoder.state_dict()
    
        # Match pre-trained weights that have same shape as current model.
        pre_train_dict_match = {
            k: v
            for k, v in pre_train_dict.items()
            if k in cross_encoder_dict and v.size() == cross_encoder_dict[k].size()
        }

        assert len(pre_train_dict)

        # Weights that do not have match from the pre-trained model.
        not_load_layers = [
            k
            for k in cross_encoder_dict.keys()
            if k not in pre_train_dict_match.keys()
        ]
        ...
        if not_load_layers:
            for k in not_load_layers:
                logger.info("Network weights {} not loaded.".format(k))
        # Load pre-trained weights.
        model.cross_encoder.load_state_dict(pre_train_dict_match, strict=False)
    else:
        breakpoint()
    
    if cfg.TRAIN.PRETRAIN == 'full':
        pre_train_dict = {k.replace("bert_encoder.model.",""):v for k,v in checkpoint["model_state"].items()}
        text_encoder_dict = model.text_encoder.state_dict()
    
        # Match pre-trained weights that have same shape as current model.
        pre_train_dict_match = {
            k: v
            for k, v in pre_train_dict.items()
            if k in text_encoder_dict and v.size() == text_encoder_dict[k].size()
        }
        ...
        not_load_layers = [
            k
            for k in text_encoder_dict.keys()
            if k not in pre_train_dict_match.keys()
        ]
        ...
        if not_load_layers:
            for k in not_load_layers:
                logger.info("Network weights {} not loaded.".format(k))
        # Load pre-trained weights.
        model.text_encoder.load_state_dict(pre_train_dict_match, strict=False)


def load_png_checkpoint(cfg,model):
    # breakpoint()
    with pathmgr.open(cfg.TRAIN.LANG_CHECKPOINT, "rb") as f:
        checkpoint = torch.load(f, map_location="cpu")

    pre_train_dict = {k.replace("bert_encoder.",""):v for k,v in checkpoint["model_state"].items()}
    model_dict = model.cross_bert.state_dict()
    ...
    if not_load_layers:
        for k in not_load_layers:
            logger.info("Network weights {} not loaded.".format(k))
    # Load pre-trained weights.
    model.cross_bert.load_state_dict(pre_train_dict_match, strict=False)
