...
class MViT(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # self.cross_bert = BertEncoder(cfg)
        ...
        self.steps = 'steps' in self.tasks
        self.grounding = any('grounding' in task for task in self.tasks)
        self.phrase = any('phrase' in task for task in self.tasks)
        ...
        for idx, task in enumerate(self.tasks):
            if 'rarp45' in task:
                ...
                # self.cross_bert = BertEncoder(cfg)
                if cfg.MODEL.LAYER_NORM:
                    self.vis_norm = nn.LayerNorm((5,768),eps=1e-3)
                    self.lang_norm = nn.LayerNorm((768),eps=1e-3)
                if cfg.MODEL.BATCH_NORM:
                    self.vis_norm = nn.BatchNorm1d(5,eps=1e-3)
                    self.lang_norm = nn.BatchNorm1d(768,eps=1e-3)

                self.text_encoder = head_helper.TextEncoder(cfg)
                self.cross_encoder = head_helper.CrossEncoder()

                extra_head = head_helper.TransformerGroundHead(
                            cfg,
                            num_classes=self.num_classes[idx],
                            dropout_rate=cfg.MODEL.DROPOUT_RATE,
                            act_func=self.act_fun[idx],
            ...
            if 'grounding_inference' == task: 
                self.add_module("extra_heads_{}".format(cfg.TRAIN.PRETRAIN_TASK.lower()+'ing'), extra_head)

                if cfg.MODEL.GROUND_LAYERS_PER_TASK:
                    self.tools_grounding_layer = nn.Linear(cfg.MODEL.NUM_TOOLS, cfg.MODEL.NUM_TOOLS)
                    self.actions_grounding_layer = nn.Linear(cfg.MODEL.NUM_ACTIONS, cfg.MODEL.NUM_ACTIONS)
                elif cfg.MODEL.GROUND_LAYERS_TASK_LAYERS:
                    self.tools_grounding_layer = nn.Linear(cfg.MODEL.MAX_PROMPTS, cfg.MODEL.NUM_TOOLS)
                    self.actions_grounding_layer = nn.Linear(cfg.MODEL.MAX_PROMPTS, cfg.MODEL.NUM_ACTIONS)
            ...
            if 'rarp45' == task[:6]:
                ...
                if task == 'grounding_inference':
                    ...
                    if self.cfg.MODEL.GROUND_LAYERS_PER_TASK:
                        ...
                        actions_affinity,_ = torch.max(affinity_mat,dim=2)
                        tools_affinity,_ = torch.max(affinity_mat,dim=3)
                        actions_affinity = self.actions_grounding_layer(actions_affinity)
                        tools_affinity = self.tools_grounding_layer(tools_affinity)
                        affinity_mat = (tools_affinity,actions_affinity)
                    elif self.cfg.MODEL.GROUND_LAYERS_TASK_LAYERS:
                        actions_affinity = self.actions_grounding_layer(affinity_mat)
                        tools_affinity = self.tools_grounding_layer(affinity_mat)
                        affinity_mat = (tools_affinity,actions_affinity)
                    ...
                    if 'phrase' in task:
                        affinity_mat = torch.bmm(out_lang,out_vis.permute((0,2,1)))
                    else:
                        affinity_mat = torch.einsum('bij,bj->bi',out_vis,lang_cls)
